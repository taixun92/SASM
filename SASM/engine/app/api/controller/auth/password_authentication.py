# -*- coding: utf-8 -*-
# 
# Script : SASM/engine/app/api/controller/auth/password_authentication.py
# Author : Hoon
#
# ====================== Comments ======================
#
...
class Process():
    ...
    def work( self, pw ):
        ...
        ##############################################################################################################
        user = g.engineDB.session.query(
                  WebUser.id
                , WebUser.pw
            )                                        \
            .filter( WebUser.id == current_user.id ) \
            .one()
        ...
        ##############################################################################################################
        g.logger.info( 'SUCCESS_PASSWORD_AUTHENTICATION' )
        ...
        return Response(
              exitcode    = SUCCESS
            , description = 'SUCCESS_PASSWORD_AUTHENTICATION'
        )